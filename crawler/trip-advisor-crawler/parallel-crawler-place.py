...
if __name__ == '__main__':
    ...
    labels['common'] = ['name','@type','priceRange','url'] 
    labels['adress'] = ['streetAddress', 'addressLocality', 'addressRegion', 'postalCode']
    labels['country']= ['country']
    labels['rating'] = ['ratingValue','reviewCount']
    ...
    for id in args.ids:
        ...
        print(":: list of itens to download fetched in " + str(delta) + "s with " + str(len(activityids)) + " elements")
            #multiprocessing
        pool = multiprocessing.Pool(processes=pool_size, initializer=ParallelCrawlerParser.start_process )#@UnusedVariable @UndefinedVariable
        job_args = []
        ...
        for activitylocationid, activityid in sorted(activityids):
            ...
            job_args.append([fields,domain, locationid, activitylocationid, activityid, 
                        basepath ,args.activity , args.timeout, args.maxretries,
                        args.maxreviews,args.pause,args.force,labels])
                
        print(":: Downloading " + locationid )
        pool_outputs = pool.map(ParallelCrawlerParser.exec_wrap, job_args )
                
        pool.close()  # no more tasks
        pool.join()  # wrap up current tasks
        
        fout = args.ids[0].split(':')[1] + ".csv"
        
        import codecs
        import csv
        
        headers = labels['common'] + labels['adress'] + labels['country'] + labels['rating']
        
        with codecs.open(os.path.join(args.out, fout), 'w', encoding='utf8') as out:
